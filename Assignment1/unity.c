...
static const char UNITY_PROGMEM UnityStrElement[]                = " Element at ";
static const char UNITY_PROGMEM UnityStrByte[]                   = " Byte at ";
static const char UNITY_PROGMEM UnityStrCharacter[]              = " Char at ";
...
void UnityPrintLen(const char* string, const UNITY_UINT32 length)
{
    ...
    if (pch != NULL)
    {
        while (((UNITY_UINT32)(pch - string) < length) && (*pch))
        {
            /* printable characters plus CR & LF are printed */
            if ((*pch <= 126) && (*pch >= 32))
            {
                            ...
                            const UNITY_LINE_TYPE lineNumber)
{
    ...
    if (expected && actual)
    {
        ...
        /* fail if expected ended but actual didn't */
        if ((!expected[i]) && actual[i])
        {
            Unity.CurrentTestFailed = 1;
            i_diff = i;
